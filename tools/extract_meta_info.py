...
# simple_dir = args.root_path + "_dwpose_simple"
...
for root, dirs, files in os.walk(args.root_path):
    for name in files:
        ...
        # if name.endswith(".mp4") and os.path.exists(os.path.join(pose_dir, name)) and os.path.exists(os.path.join(simple_dir, name)):
            video_mp4_paths.add(os.path.join(root, name))
video_mp4_paths = list(video_mp4_paths)

print(f"Total number of videos: {len(video_mp4_paths)}")
...
for video_mp4_path in video_mp4_paths:
    ...
    # simple_kps_path = os.path.join(simple_dir, relative_video_name)
    meta_infos.append({"video_path": video_mp4_path, "kps_path": kps_path})
    # meta_infos.append({"video_path": video_mp4_path, "kps_path": kps_path, "simple_kps_path": simple_kps_path})
