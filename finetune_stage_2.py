...
from src.dataset.dance_video import HumanDanceVideoDataset_fast
...
def log_validation(
    ...
    test_cases = [
        (
            "./configs/inference/ref_images/1_new.png",
            "./configs/inference/pose_videos/0010_2.mp4",
        ),
        (
            "./configs/inference/ref_images/2_new.png",
            "./configs/inference/pose_videos/0010_2.mp4",
        ),
    ]
...
def main(cfg):
    ...
    if cfg.stage1_ckpt_dir is not None:
    ...
    train_dataset = HumanDanceVideoDataset_fast(
    ...
    for epoch in range(first_epoch, num_train_epochs):
                    ...
                    if accelerator.is_main_process:
                        ...
                        for sample_id, sample_dict in enumerate(sample_dicts):
                            ...
                            with TemporaryDirectory() as temp_dir:
                                out_file = Path(
                                    f"/home/jgkwak/Moore-AnimateAnyone/Moore-AnimateAnyone/stage2_val/{global_step:06d}-{sample_name}.gif"
                                )
        ...
        if accelerator.is_main_process and global_step % cfg.val.validation_steps == 0:
            save_path = os.path.join(save_dir, f"checkpoint-{global_step}")
            delete_additional_ckpt(save_dir, 1)
            accelerator.save_state(save_path)
            # save motion module only
