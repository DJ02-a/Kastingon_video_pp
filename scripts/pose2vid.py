...
from src.utils.util import get_fps, read_frames, read_frames_cv, save_videos_grid
...
def main():
    ...
    for ref_image_path in config["test_cases"].keys():
        ...
        for pose_video_path in config["test_cases"][ref_image_path]:
            ...
            pose_list = []
            pose_tensor_list = []
            pose_images = read_frames_cv(pose_video_path)
            ...
            total_frame = min(len(pose_images)-1, args.L)
            for pose_image_pil in pose_images[: total_frame]:
            ...
            ref_image_tensor = repeat(
                ref_image_tensor, "b c f h w -> b c (repeat f) h w", repeat=total_frame
            )
            ...
            print("saved video to", f"{save_dir}/{ref_name}_{pose_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.mp4")
